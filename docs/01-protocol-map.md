...
3. <- PairingStatus  flags0xC0 07/45  payload:
        0x00 0x01 = already paired    -> done
        0x00 0x02 = approval required -> camera shows dialog/code on screen
4. (if 0x02) user approves on camera
   <- PairingPINApproved flags0x40 07/46  payload 0x01
   <- Pair               flags0xC0 07/46  payload 0x00
```
...
`pin` — **this is the unknown for the Nano.** Three possibilities we design for:
- (a) **Vestigial**: older/streaming cameras accept a fixed string (`love`, `5160`); the
      real gate is the on-screen *approve* tap. Try this first.
- (b) **Real numeric PIN** shown on the Nano screen that must be echoed in `SetPairingPIN`
      → we need a text field in the UI to enter it.
- (c) Camera rejects/È returns a status we haven't catalogued → log raw and iterate.

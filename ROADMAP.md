...
- **Write (control).** Two routes. On **R-SDK** (#9) the control commands are documented and clean —
  ...
  UI on top of the R-SDK session is the small remaining piece there. On the **media path**, the ids are RE'd
  from open source (`reference/osmo-download/src/osmo_download/duml.py`, camera cmdset `0x02`): take-photo
  `0x02/0x01`, start-record `0x02/0x20`, stop-record `0x02/0x21`, set-mode `0x02/0x02` — but firmware-derived,
  **unverified on the Nano/Xtra**, payloads unmapped, over the same UDP datalink as the file list.
